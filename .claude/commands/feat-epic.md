...
## Decisions so far   — one line per closed ticket: - [<title>](tickets/NN-name.md) — <gist>
...
## Question
## Resolution         — added on close: the answer; assets linked, not pasted
```
...
4. Record it: write the Resolution, set Status `closed`, append its line to Decisions so far.
5. Update the map: create newly surfaced tickets (then wire them); graduate fog the answer
   made specifiable, removing it from Not yet specified; rule a ticket beyond the
   Destination out of scope (Status `out-of-scope` + its line in Out of scope — never in
   Decisions so far); rewrite or delete tickets the decision invalidated.
6. **Clear the map** when no open ticket and no fog remain: agree the split in one final
   ...
   `/feat-new <slug> <one-line description> [epic: $1]` + `after: <slugs>` if any.
...
## Close every run
Name the files written, then the next command: `/feat-epic $1` while Status is `charting`;
once `cleared`, the first `/feat-new` line of Feature breakdown. End with this reminder,
verbatim: "Epic and feature slugs share one namespace — a `/feat-new` slug may not be
