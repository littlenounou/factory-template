...
5. Update `state.json` step to `backend`. Tell the user the next step is `/feat-frontend $1` (if frontend enabled) else `/feat-verify $1` — or, FABLE 5: once all enabled build steps are done, `/feat-ship $1` to converge verify→validate→fix under one `/goal`.
